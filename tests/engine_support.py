...
# tools.fixture_utils is a dev-only module; import lazily so this module can
...
def connect_engine() -> Any:
    ...
    Reads ``ENGINE_DB_*`` environment variables for host, port, user, database,
    and password; delegates to :func:`tools.fixture_utils.connect_dsn` which is
    the single place ``mssql_python.connect`` is called in the test tooling.
    ...
    port = int(os.environ.get("ENGINE_DB_PORT", "30004"))
    ...
    try:
        return fixture_utils.connect_dsn(
