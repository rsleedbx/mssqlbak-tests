...
# TDE page-decryption fixture (database-level TDE, backup NOT backup-level encrypted).
# Generated by: python -m tools.fixture_run tde-page
FIXTURE_BAK_TDE_PAGE = _FIXTURE_DIR / "tde_page_full.bak"
FIXTURE_BAK_TDE_PLAIN = _FIXTURE_DIR / "tde_page_plain.bak"
FIXTURE_TDE_PAGE_PFX = _FIXTURE_DIR / "tde_page_cert.pfx"
FIXTURE_TDE_PAGE_CERT_PASSWORD = "TdePageCert!Fixture2024"

# Backup-level WITH ENCRYPTION fixtures.
# Generated by: python -m tools.fixture_run enc-bak
FIXTURE_ENC_BAK_PLAIN   = _FIXTURE_DIR / "enc_bak_plain.bak"
FIXTURE_ENC_BAK_AES128  = _FIXTURE_DIR / "enc_bak_aes128_full.bak"
...
@pytest.fixture
def fixture_bak_tde_page() -> Path:
    """Path to the TDE page-decryption fixture (database-level TDE only).

    Generate with::

        python -m tools.fixture_run tde-page
    """
    if not FIXTURE_BAK_TDE_PAGE.exists():
        pytest.skip(
            f"TDE page fixture missing: {FIXTURE_BAK_TDE_PAGE} "
            "(run: python -m tools.fixture_run tde-page)"
        )
    return FIXTURE_BAK_TDE_PAGE


@pytest.fixture
def fixture_bak_tde_plain() -> Path:
    """Path to the plaintext (no-TDE) backup paired with the TDE page fixture."""
    if not FIXTURE_BAK_TDE_PLAIN.exists():
        pytest.skip(
            f"TDE plain fixture missing: {FIXTURE_BAK_TDE_PLAIN} "
            "(run: python -m tools.fixture_run tde-page)"
        )
    return FIXTURE_BAK_TDE_PLAIN


@pytest.fixture
def fixture_tde_page_pfx() -> Path:
    """Path to the PFX certificate for the TDE page fixture."""
    if not FIXTURE_TDE_PAGE_PFX.exists():
        pytest.skip(
            f"TDE page PFX missing: {FIXTURE_TDE_PAGE_PFX} "
            "(run: python -m tools.fixture_run tde-page)"
        )
    return FIXTURE_TDE_PAGE_PFX
...
_ENC_BAK_RUN_HINT = "python -m tools.fixture_run enc-bak"


@pytest.fixture
def fixture_enc_bak_plain() -> Path:
    """Path to the plaintext (no encryption) reference backup for enc-bak fixtures."""
    if not FIXTURE_ENC_BAK_PLAIN.exists():
        pytest.skip(
            f"enc-bak plain fixture missing: {FIXTURE_ENC_BAK_PLAIN} "
            f"(run: {_ENC_BAK_RUN_HINT})"
        )
    return FIXTURE_ENC_BAK_PLAIN


@pytest.fixture
def fixture_enc_bak_aes128() -> Path:
    """Path to the AES_128 backup-level encrypted backup."""
    if not FIXTURE_ENC_BAK_AES128.exists():
        pytest.skip(
            f"enc-bak AES_128 fixture missing: {FIXTURE_ENC_BAK_AES128} "
            f"(run: {_ENC_BAK_RUN_HINT})"
        )
    return FIXTURE_ENC_BAK_AES128


@pytest.fixture
def fixture_enc_bak_aes256() -> Path:
    """Path to the AES_256 backup-level encrypted backup."""
    if not FIXTURE_ENC_BAK_AES256.exists():
        pytest.skip(
            f"enc-bak AES_256 fixture missing: {FIXTURE_ENC_BAK_AES256} "
            f"(run: {_ENC_BAK_RUN_HINT})"
        )
    return FIXTURE_ENC_BAK_AES256


@pytest.fixture
def fixture_enc_bak_aes256_compressed() -> Path:
    """Path to the AES_256 + COMPRESSION backup-level encrypted backup."""
    if not FIXTURE_ENC_BAK_AES256C.exists():
        pytest.skip(
            f"enc-bak AES_256+COMPRESSION fixture missing: {FIXTURE_ENC_BAK_AES256C} "
            f"(run: {_ENC_BAK_RUN_HINT})"
        )
    return FIXTURE_ENC_BAK_AES256C


@pytest.fixture
def fixture_enc_bak_pfx() -> Path:
    """Path to the PFX certificate for enc-bak fixtures."""
    if not FIXTURE_ENC_BAK_PFX.exists():
        pytest.skip(
            f"enc-bak PFX missing: {FIXTURE_ENC_BAK_PFX} "
            f"(run: {_ENC_BAK_RUN_HINT})"
        )
